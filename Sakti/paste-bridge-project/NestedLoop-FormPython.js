...
(function () {
        ...
        try {
            ...
            for (let i = 0; i < detailRows.length; i++) {
                ...
                if (inputNilai && nilaiFormatted) {
                    inputNilai.focus();
                    await sleep(300); // tunggu fokus

                    // Kirim ke file lokal (via fake form / API / custom solusi)
                    fetch("http://localhost:3030/paste", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ nilai: nilaiFormatted }),
                    });

                    console.log("📤 Kirim nilai ke Python:", nilaiFormatted);
                    await sleep(1500); // beri waktu Python paste
                }
    ...
    async function runAutomationWithDelay() {
        ...
        if (nextBtn && totalRowProcessed < 7) {
            nextBtn.click();
            await sleep(3000);
