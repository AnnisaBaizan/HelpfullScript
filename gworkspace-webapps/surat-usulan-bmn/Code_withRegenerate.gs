...
// ============================================================
//  GET ARSIP SURAT
//  Return semua data surat untuk dropdown regenerate
// ============================================================
function getArsip() {
  try {
    ...
    if (lastRow < 2) return { status: 'ok', arsip: [] };

    const data = sheet.getRange(2, 1, lastRow - 1, 20).getValues();
    const arsip = data
      .filter(r => r[1]) // hanya baris yang ada nomor surat
      .map(r => ({
        nomor          : r[1],   // B: Nomor Surat
        tanggalSurat   : r[3] ? Utilities.formatDate(new Date(r[3]), Session.getScriptTimeZone(), 'yyyy-MM-dd') : '',
        nama           : r[4],   // E: Nama
        nip            : r[5],   // F: NIP
        jabatan        : r[6],   // G: Jabatan
        bagian         : r[7],   // H: Unit/Bagian
        namaBarang     : r[8],   // I: Nama Barang
        merekTipe      : r[9],   // J: Merek/Tipe
        ruangan        : r[10],  // K: Ruangan
        nup            : r[11],  // L: NUP
        kondisi        : r[12],  // M: Kondisi
        keluhan        : r[13],  // N: Keluhan
        linkNup        : r[14],  // O: Link Foto NUP
        linkMerek      : r[15],  // P: Link Foto Merek
        linkKerusakan  : r[16],  // Q: Link Foto Kerusakan
        linkKeseluruhan: r[17],  // R: Link Foto Keseluruhan
      }));

    return { status: 'ok', arsip: arsip };
  } catch (err) {
