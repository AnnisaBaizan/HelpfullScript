...
function kirimEmailAlertED() {
  ...
  if (!sheet) {
    Logger.log("❌ Sheet '" + CONFIG.NAMA_SHEET + "' tidak ditemukan.");
    return;
  }
  if (!ringkasan) {
    Logger.log("❌ Sheet 'RINGKASAN' tidak ditemukan.");
    return;
  }

  // ── Baca threshold dinamis dari RINGKASAN ────────────────────
  const batasPerhatian = ringkasan
    .getRange(CONFIG.SEL_BATAS_PERHATIAN)
    .getValue();
  ...
  if (batasKritis >= batasPerhatian) {
    Logger.log(
      "❌ Konfigurasi threshold tidak valid: D6 harus lebih kecil dari D4.",
    );
    Logger.log(
      "   D4 (PERHATIAN): " +
        batasPerhatian +
  ...
  const totalAlert =
    kelompok.expired.length +
    kelompok.kritis.length +
    kelompok.perhatian.length;

  // Jika tidak ada yang perlu di-alert → tidak kirim email
  if (totalAlert === 0) {
    Logger.log(
      "✅ Semua bahan aman (stok > 0, sisa hari > " +
        batasPerhatian +
        "). Email tidak dikirim.",
    );
    return;
...
function buatBodyEmail(
  ...
  const tglHariIni = Utilities.formatDate(
    today,
    "Asia/Jakarta",
    "dd MMMM yyyy",
  );
  ...
  const styleTable =
    "width:100%;border-collapse:collapse;font-family:Arial,sans-serif;font-size:13px;margin-bottom:16px;";
  const styleTh =
    "background:#1a237e;color:#fff;padding:8px 10px;text-align:left;font-size:12px;";
  ...
  function buatTabel(items, warnaHeader) {
    ...
    const rows = items
      .map(function (b) {
        const expStr = Utilities.formatDate(
          b.tglExpired,
          "Asia/Jakarta",
          "dd/MM/yyyy",
        );
  ...
  const sectionExpired =
    kelompok.expired.length > 0
      ? "<h3 style='color:#b71c1c;margin:20px 0 8px;'>⛔ EXPIRED (" +
        kelompok.expired.length +
        " bahan) — Segera Pisahkan & Jangan Digunakan</h3>" +
        buatTabel(kelompok.expired, "#c62828")
      : "";

  const sectionKritis =
    kelompok.kritis.length > 0
      ? "<h3 style='color:#e65100;margin:20px 0 8px;'>🔴 KRITIS — ED ≤ " +
        batasKritis +
        " Hari (" +
        kelompok.kritis.length +
        " bahan) — Prioritaskan Penggunaan</h3>" +
        buatTabel(kelompok.kritis, "#e64a19")
      : "";

  const sectionPerhatian =
    kelompok.perhatian.length > 0
      ? "<h3 style='color:#f57f17;margin:20px 0 8px;'>🟡 PERHATIAN — ED " +
        (batasKritis + 1) +
        "–" +
        batasPerhatian +
        " Hari (" +
        kelompok.perhatian.length +
        " bahan) — Pantau Rutin</h3>" +
        buatTabel(kelompok.perhatian, "#f57c00")
      : "";
  ...
  return [
    ...
    "  <p style='margin:6px 0 0;font-size:13px;opacity:0.85;'>" +
      ...
      CONFIG.INSTITUSI +
      "</p>",
    "  <p style='margin:4px 0 0;font-size:12px;opacity:0.7;'>Tanggal Laporan: " +
      tglHariIni +
      "</p>",
    "</div>",
    ...
    "    <td><b>🟡 Perhatian</b> (≤" +
      ...
      kelompok.perhatian.length +
      " bahan</td>",
    "    <td><b>Total Alert:</b> <span style='color:#c62828;font-weight:bold;'>" +
      totalAlert +
      " bahan</span></td>",
    "  </tr></table>",
    "</div>",
    ...
    "  <p>Email ini dikirim otomatis oleh Sistem Monitoring ED · " +
      CONFIG.NAMA_LAB +
      "</p>",
